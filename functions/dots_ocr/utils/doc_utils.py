...
def load_images_from_pdf_bytes(pdf_bytes, dpi=200, start_page_id=0, end_page_id=None) -> list:
    images = []
    # 直接从内存字节流打开，而不是文件路径
    with fitz.open(stream=pdf_bytes, filetype="pdf") as doc:
        pdf_page_num = doc.page_count
        end_page_id = end_page_id if end_page_id is not None and end_page_id >= 0 else pdf_page_num - 1
        end_page_id = min(end_page_id, pdf_page_num - 1)  # 防止溢出

        for index in range(start_page_id, end_page_id + 1):
            page = doc[index]
            pix = page.get_pixmap(dpi=dpi)  # 替代你原来的 fitz_doc_to_image
            img = pix.pil_tobytes("png")  # 得到可直接展示/保存的图片 bytes
            images.append(img)
